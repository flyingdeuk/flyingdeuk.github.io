---
title: ZSQD(TAO) - 중국 칭다오(자오동 공항) <2025.11.23 Updated>
date: 2023-05-20
categories: [Flying]
tags: [china, tao]
pin:
...
- IN/OUT : 대략 1시간 10분. 
- 가끔 After AGAVO DCT IKEKA then LATUX 주기도 함. 
...
- ATIS에 Single RWY Operation (아침비행)으로 명시되나 이후에 다시 변경됨. (서쪽 RWY L/D, 동쪽 RWY 이륙) -> 아침 출발전에는 RWY16/34 ATIS에 적혀있으나 이내 RWY 17/35로 변경된다. 

## APP
- 통상적인 Initial APP 속도 210kts가 Chart에 명시됨. 
- Traffic에 따라 STAR 도중에 RDR Vector로 관제함. 
...
- ShutDown이후 GPU와 Air Conditon 연결확인 후 절차 수행이 필요함. PACKs - OFF (Quick Turn시 절차수행 특별히 안함) -> A330 APU off에 대한 명시없음.
...
## SID
- DCL 가능하나 기준시간에 대한 언급은 없음 DCL에 ADT는 단지 DCL 요청시간임. 
- Voice ATC CLR는 10분전 이후에 하라고 되어있음. 
- ATC Hold에 대비해서 연료 보급 및 DCL Req 빨리하는 것이 추천됨. -> No Read Back으로 변경됨. 
- 이륙후 H/D지시이후 W209 Join하라는 경우 대비 필요 (CRS148/Radial IN 328)

## GND
- Apron 내에 주파수 변경을 위한 HP(Holding Point)에 주의가 필요함. 
- 이륙후 이륙활주로와 SID 보고
- GND CTC 없이 Apron에서 TWR로 바로 넘겨줌. 
