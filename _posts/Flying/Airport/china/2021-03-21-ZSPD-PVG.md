---
title: ZSPD(PVG) - 중국 상하이(푸동 공항) <2026.6.2 Updated>
date: 2021-03-21
categories: [Flying]
tags: [china, pvg]
pin:
...
# PREFLT
- 중국 모든 QR 절차는 사라짐. 여권 번호확인 및 Custom Form 작성만 하면됨.  
...
![pvg](/img/flying/airport/pvgp02.jpg)
- P02 Route로 파일되는 경우도 있음. 
...
- DCL 10분전으로 변경되었음. 가능하며 불러줄 필요없이 We have DCL 정도면 끝나게 절차가 변경됨. 
- 북쪽 북경공항등 유럽 트래픽이 많은 공항과는 다르게 A-CDM 절차가 엄격하지 않아 ADT나 TSAT, CTOT등이 고려사항이 아님. (시간이 다르게 나오나 그냥 준비되면 REQ하면 됨) -> DCL이 ENG Start up 10분 전으로 변경이 되었으며... ADT는 DCL Send time이 찍힘... 의미없는 시간임. 
...
- 통상적인 딜레이는 PUSH BACK이 늦어서가 아니라 이륙전 많은 Traffic으로 인한 딜레이 인듯하다. (특히 PVG-PUS의 경우 5분 빨리 PushBack하였으나... 이륙까지 30분이 넘게 걸림.)
...
## Immigration
- INBD : 기내 or 비행중 저녁 식사후 호텔로 이동. 직원 Escort가 없어짐. 알아서 나가야함. 
- OUTBD : 호텔 조식 or 도시락 후 차량탑승 - Gate 14 - Koreanair Counter H1 - 직원 Escort하에 이동

## [Ramada Pudong Airport Shanghai](https://maps.app.goo.gl/gJDU4EGTJtpP5Bu67)(Click)
- 아주 오래전 이용하던 공항 바로 옆의 호텔
- 주변에 아무것도 없음. 
- 아침 조식 또는 도시락 선택이 가능함. (CheckIn시 인원 알려 달라고함.)

`최소 휴식시간 10시간 보장 기준으로 RAMPIN-STD 10시간 이내로 Delay시 DEP시간 조절해야하며 지점에서 연락은 오지만 PICKUP/WAKEUP 시간은 기장이 조절해서 알려줘야함`

- WIFI : CHK IN시 코드를 입력해야 인터넷 연결됨. 
  - 비행준비를 위한 인터넷 연결은 Flexiroam 셀룰러로 하거나 회사 VPN 연결하면 가능하다. 
...
- YOUTUBE, Netflix 보는 법
  - 회사 VPN 연결보다는 개인것을 연결하는 것을 추천한다. 
...
# PLACE
호텔 주변이 주거지역으로 식당과 슈퍼가 가까움. 

![pvg](/img/flying/airport/pvgplace.jpg)

![pvg](/img/flying/airport/pvgplace.png)
- 영어가 전혀 통하지않으며 파파고나 구글 번역기로 소통이 가능함. 
- 그냥 바구니에 먹고싶은 것을 담아서 줘도 알아서 구워줌.
- 현금이나 알리페이로 지불이 가능함. (알리페이 어플 설정하면 편함.) -> 알페이 결재시 3개 옵션중에 맨 아래 알리페이로 결재를 선택해야 수수료없이 결재가 가능하다. 신용카드 결재는 수수료를 지불해야함. 
- 가지를 선택해서 주면 사진과 같은 가지 요리를 해줌...
...
- 20-25분 거리에 위치함. 
- 주변에 식당 및 편의점이 있으나 카드사용이 안됨. 미리 위안화를 준비해서 가거나 알리페이 어플 이용이 가능함. 
- 아침 조식 또는 도시락 선택이 가능함. (CheckIn시 인원알려달라고함.)
- PICKUP 1시간 50분전(0700L) 기준으로 2시간50분전(0600L) WAKEUP CALL 
