---
title: RPLL(MNL) - 필리핀 마닐라(니노이 아키노 공항) <2026.7.31 Updated>
date: 2020-08-31
categories: [Flying]
...
## INBD
- ~~ATS PLAN과 PLAN Sign을 공항해서 Hard Copy로 하는 공항임.~~ -> Hard Copy 사라짐. 
- 도착시 받은 GD 챙겨야함. (스테이션 인포에 붙어있음)
...
## **Japan RTE** 
- B737 : 3시간 40-50분 정도, A330 : 3시간 20-30분
- "SENKA" 20NM Wx deviation시 주의 필요.
...
- RWY24 : Vacate Point를 알려줌. 안되면 보고. Vacate이후 바로 Apron CTC함. Gate까지 짧음. 
- RNP를 Primary로 사용함. -> ILS 사용함. (RNAV STAR와 연결되지않으나 ILS 사용함)
- 이륙항공기와의 간격을 좁게 관제해서 착륙시 전방 이륙 항공기의 Wake Turbulance를 주의해야함. 
...
- CHOCK IN Time 물어보는 경우 있음
- Gate 6 고정식 브리지 정확한 정지가 필요함. 
...
## Walkaround
- Gate 6 : L2 비번 없이 나감. 
> GND Staff가 블루투스 헤드셋을 계속 끼고 있어 OXY TEST시 필히 말해줘야함. 
...
- DEP FREQ 124.4
- 이륙전 CPDLC LOG ON RPHI 추가됨. 

## GND
- BLOCK OFF Time 물어보는 경우도 있음. 
...
# STAY
## Immigration
- INBD : 검역 세관신고서 QR Code 마지막 제시, 직원이 IN/OUT GD가 붙어있는 Info를 줌. IN 제시 필요한 경우 있음. 
- OUTBD : Crew Line으로 진입해 여권검사 짐검사 (도착시 반은 OUT GD제출)


## [Dusit Thani Hotel Manila](https://maps.app.goo.gl/zuwPH45KY1ju2V1F7)(Click)
- 16층 배정 (Executive Floor) Lounge 이용가능 -> 그때그때 다름
- 17층 Executive Lounge : 음식 괜찬은 편이고 주류도 Free
  - Breakfast : 6:00AM - 10:30AM
    - 1층 : 다양한 음식이 있음. 누들 Station은 별루...
    - 17층 : Egg 요리 주문이 가능하며 음식 숫자는 좀 적음. 
